...
// }
// const url =
// 	process.env.APP_URL || 'https://weather-bot-mezgoodle.herokuapp.com:443'
const bot = new TelegramBot(Token, { polling: true })
...
const weatherEndpoint = (lat, lon, lang) =>
	`https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&exclude=hourly,minutely&appid=${ApiKey}&units=metric&lang=${lang}`
		...
		resp => {
			...
			for (let i = index[0]; i <= index[1]; i++) {
				...
				bot.sendPhoto(chatId, weatherIcon(daily[i].weather[0].icon), {
					caption: weatherHTMLTemplate(daily[i], date, city),
					parse_mode: 'HTML',
				})
			}
		},
...
bot.onText(/\/set (.+)/, (msg, match) => {
	...
	User.findOneAndUpdate({ user_id }, { city }, (err, res) => {
		if (err) {
			bot.sendMessage(
				`Sorry, but now function is not working.\n\r Error: ${err}`
			)
		} else if (res === null) {
			...
			new_user
				...
				.catch(() => {
					bot.sendMessage(
						chatId,
						`${msg.from.first_name}, sorry, but something went wrong`
					)
				})
		} else {
			bot.sendMessage(
				chatId,
				`${msg.from.first_name}, your information has been updated`
			)
		}
		return
	})
...
bot.onText(/\/lang (.+)/, (msg, match) => {
	...
	User.findOneAndUpdate({ user_id }, { lang }, (err, res) => {
		if (err) {
			...
			bot.sendMessage(
				chatId,
				`${msg.from.first_name}, please first type /set command`
			)
		} else {
			bot.sendMessage(
				chatId,
				`${msg.from.first_name}, your language has been updated`
			)
		}
		return
	})
...
const getInfo = (chatId, user_id, city, index) => {
	if (city) {
		axios
			.get(
				`https://api.opencagedata.com/geocode/v1/json?q=${city}&key=${GeoapiKey}&pretty=1`
			)
			.then(
				resp => {
		...
		User.findOne({ user_id })
			.then(doc => {
				if (doc) {
					axios
						.get(
							`https://api.opencagedata.com/geocode/v1/json?q=${doc.city}&key=${GeoapiKey}&pretty=1`
						)
						.then(
							resp => {
								const { lat, lng } = resp.data.results[0].geometry
